...
function sslp_staff_member_info_meta_box(){
    ...
    $_staff_member_office = $custom["_staff_member_office"][0];
    $_staff_member_website = $custom["_staff_member_website"][0];
	...
	<div class="sslp_admin_wrap">
        ...
        <label for="_staff-member-office">Office: <input type="text" name="_staff_member_office" id="_staff_member_office" placeholder="<?php if ($_staff_member_office == '') echo ('Staff Member\'s Office'); ?>" value="<?php if ($_staff_member_office != '') echo $_staff_member_office; ?>" /></label>
		<label for="_staff-member-website">Website: <input type="text" name="_staff_member_website" id="_staff_member_website" placeholder="<?php if ($_staff_member_website == '') echo ('Staff Member\'s Website'); ?>" value="<?php if ($_staff_member_website != '') echo $_staff_member_website; ?>" /></label>
        <label for="_staff-member-fb">Facebook: <input type="text" name="_staff_member_fb" id="_staff_member_fb" placeholder="<?php if ($_staff_member_fb == '') echo ('Staff Member\'s Facebook URL'); ?>" value="<?php if ($_staff_member_fb != '') echo $_staff_member_fb; ?>" /></label>
...
function sslp_staff_member_display_custom_columns( $column ) {
  ...
  switch ( $column ) {
    ...
    case "_staff_member_phone":
      ...
      break;
    case "_staff_member_office":
      echo $_staff_member_office;
      break;
    case "_staff_member_website":
      echo $_staff_member_website;
      break;
		...
		<table class="wp-list-table widefat fixed posts" id="sortable-table">
			<thead>
				<tr>
                    ...
                    <th class="column-office">Office</th>
                    <th class="column-website">Website</th>
			...
			?>
				<tr id="post-<?php the_ID(); ?>">
                    ...
                    <td class="column-office"><?php echo $custom["_staff_member_office"][0]; ?></td>
                    <td class="column-website"><?php echo $custom["_staff_member_website"][0]; ?></td>
...
function sslp_staff_member_usage_page() {
	...
	$output .= '<h3>Custom Changes</h3>';
    $output .= '<p>This plug-in has been modified by Kevin D. Bannon for use on the various websites under the College of Arts and Media. <strong>DO NOT UPDATE THIS PLUG-IN</strong> or all changes will be lost.</p>';
    $output .= '<p>1. The default template has been modified to reflect suggested usage on the campus of Marshall University and takes full advantage of the responsive design of the website.</p>';
    $output .= '<p>2. The "office" field has been added to the plug-in. The template tag is <code>[staff-office]</code></p>';
    $output .= '<p>3. The "website" field has been added to the plug-in. The template tag is <code>[staff-website]</code></p>';
    $output .= '<p>4. The "Twitter Username" field was originally part of the plug-in, but the <code>https://twitter.com</code> portion of the link has been removed from the code.  Users must now include the entire link to their page.  This makes it easier to copy and paste from a web browser.</p>';
    $output .= '<p>5. The "<code>[staff-mu-custom]</code>" tag has been added. When used in the template, an unordered list will be generated that includes website, facebook, and twitter links.</p>';
    $output .= '<p>6. The "<code>[staff-bio-formatted]</code>" tag will now hide the bio information and display a button to "View Bio." No button will appear if there is no information.</p>';
    $output .= '<p>7. The feature to force all phone numbers to display in the same format has been re-activated.  Phone numbers MUST be ten digits long!</p>';
    $output .= '<p>8. A template page for displaying the bio for a single person has been added and must be placed in the templates directory.  The name of the page is <code>single-staff-member.php</code></p>';
...
function sslp_staff_member_template_page(){ 
    ...
    $output .= '<p>**Note: The tag <strong>[staff-mu-custom]</strong> will output an unordered list of website, twitter, and facebook links.</p>';
